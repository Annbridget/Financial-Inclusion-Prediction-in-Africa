...
if st.button("Predict"):
    ...
    # Display result
    if prediction[0] == 1:
        st.success(f"The individual is likely to have a bank account (confidence: {probability[0][1]*100:.2f}%)")
    else:
        st.warning(f"The individual is unlikely to have a bank account (confidence: {probability[0][0]*100:.2f}%)")